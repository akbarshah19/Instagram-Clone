...
//        spinner.startAnimating()
    ...
    private func fetchNotifications() {
        for x in 0...20 {
            let post = UserPost(id: "", postType: .photo, thumbnailImage: URL(string: "https://www.google.com")!, postURL: URL(string: "https://www.google.com")!, caption: nil, likeCounts: [], comments: [], createdDate: Date(), taggedUsers: [])
            let model = UserNotification(type: x % 2 == 0 ? .like(post: post) : .follow(state: .not_following),
                                         text: "Hello World",
                                         user: User(username: "joe_smoth",
                                                    bio: "",
                                                    name: (first: "", last: ""),
